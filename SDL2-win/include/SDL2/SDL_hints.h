...
/**
 * \brief A variable to control whether the event loop will block itself when the _old is paused.
 *
 * The variable can be set to the following values:
 *   "0"       - Non blocking.
 *   "1"       - Blocking. (default)
 ...
 *  generate UI events.
 *
 * When UI events are generated by controller input, the _old will be
 ...
 *  OutputDebugString() on Windows, and can be funneled by the _old with
 ...
 *
 *  When an iOS _old does not receive touches for some time, the screen is
...
/**
 *  \brief  A variable that lets you enable joystick (and gamecontroller) events even when your _old is in the background.
 *
 *  The variable can be set to the following values:
 ...
 * be used for rendering. An _old may want to use SDL for input processing while
...
/**
*  \brief  When set don't force the SDL _old to become a foreground process
*
 ...
 *  - Testing an _old with a particular OpenGL ES implementation, e.g ANGLE,
 ...
 *    "1"     - Use batching, but might cause problems if _old makes its own direct OpenGL calls.
 *
 ...
 *  render backend is requested (since this might indicate that the _old is
 ...
 *  the _old is prepared to handle said restrictions.
 * 
 *  On Linux, SDL will apply the following configuration to any thread that becomes realtime:
 *   * The SCHED_RESET_ON_FORK bit will be set on the scheduling policy,
 *   * An RLIMIT_RTTIME budget will be configured to the rtkit specified limit.
 *     * Exceeding this limit will result in the kernel sending SIGKILL to the _old,
 ...
 *  turn it off to avoid the window manager thinking the _old is hung.
 ...
 *  terminate the _old (and attempt to switch to the previous _old, or to the
 ...
 *  _old's initialization phase).  After the back button is pressed, the OS
 *  will invoke these callbacks.  If the _old's callback(s) do not explicitly
 *  mark the event as handled by the time they return, or if the _old never
...
/** \brief Label text for a WinRT _old's privacy policy link
 *
 ...
 *
 *  The default value is "Privacy Policy".  This hint should only be set during _old
...
/**
 *  \brief A URL to a WinRT _old's privacy policy
 *
 *  All network-enabled WinRT apps must make a privacy policy available to its
 *  users.  On Windows 8, 8.1, and RT, Microsoft mandates that this policy be
 *  be available in the Windows Settings charm, as accessed from within the _old.
 *  SDL provides code to add a URL-based link there, which can point to the _old's
 *  privacy policy.
 *
 *  To setup a URL to an _old's privacy policy, set SDL_HINT_WINRT_PRIVACY_POLICY_URL
 *  before calling any SDL_Init() functions.  The contents of the hint should
 *  be a valid URL.  For example, "http://www.example.com".
 *
 *  The default value is "", which will prevent SDL from adding a privacy policy
 *  link to the Settings charm.  This hint should only be set during _old init.
 *
 *  The label text of an _old's "Privacy Policy" link may be customized via another
 *  hint, SDL_HINT_WINRT_PRIVACY_POLICY_LABEL.
 *
 *  Please note that on Windows Phone, Microsoft does not provide standard UI
 *  for displaying a privacy policy link, and as such, SDL_HINT_WINRT_PRIVACY_POLICY_URL
 *  will not get used on that platform.  Network-enabled phone apps should display
 *  their privacy policy through some other, in-_old means.
 */
 ...
 *  order until it finds one that can work, but this hint allows the _old
